const { test } = require('@playwright/test');
const axios = require('axios');

// Define the access token and API URL for Microsoft Graph
const accessToken = ''; // Replace with your actual access token
const apiUrl = 'https://graph.microsoft.com/v1.0/me/calendars';

// Axios instance to measure response time
const axiosInstance = axios.create();

axiosInstance.interceptors.request.use(config => {
  config.metadata = { startTime: new Date() };
  return config;
});

axiosInstance.interceptors.response.use(response => {
  response.config.metadata.endTime = new Date();
  response.duration = response.config.metadata.endTime - response.config.metadata.startTime;
  return response;
}, error => {
  error.config.metadata.endTime = new Date();
  error.duration = error.config.metadata.endTime - error.config.metadata.startTime;
  return Promise.reject(error);
});

// Function to get the count of calendars with pagination
async function getCalendarsCount() {
  let totalCalendars = 0;
  let nextLink = apiUrl;

  try {
    while (nextLink) {
      const response = await axiosInstance.get(nextLink, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
      const calendars = response.data.value;
      totalCalendars += calendars.length;
      nextLink = response.data['@odata.nextLink'];
    }
    console.log(`Total number of calendars: ${totalCalendars}`);
    return totalCalendars;
  } catch (error) {
    console.error('Failed to get calendars count:', error.response?.data || error.message);
    return 0;
  }
}

// Playwright test to get and log the count of calendars
test('Get and log the count of calendars', async ({}) => {
  test.setTimeout(36000000); // Set the test timeout to 1 hour (3600000 ms)
  const count = await getCalendarsCount();
  console.log(`Total number of calendars: ${count}`);
});
